...
if [ -f /jffs/pptp/manual/domainname.conf ]; then
    ...
    echo $(ping -w 1 -c 1 $(cat /jffs/pptp/manual/domainname.conf) | grep -Eo "\(([0-9.]+)" | cut -d\( -f 2) >> /jffs/pptp/manual/ipaddress.conf
    if [ $(cat /jffs/pptp/manual/ipaddress.conf) == $(ping -w 1 -c 1 domainnotexists | grep -Eo "\(([0-9.]+)" | cut -d\( -f 2) ]; then
        rm -f /jffs/pptp/manual/ipaddress.conf
